...
def _clean_db_url(url):
    """Nettoie l'URL Prisma/Vercel pour pg8000 :
    - Remplace 'prisma+postgres://' par 'postgres://'
    - Retire les paramètres pgbouncer, connect_timeout, etc.
    - Garde uniquement sslmode=require si présent
    """
    if not url:
        return url
    ...
    p = urlparse(url)
    # Garder uniquement sslmode dans les query params
    qs = parse_qs(p.query)
    clean_qs = {}
    if "sslmode" in qs:
        clean_qs["sslmode"] = qs["sslmode"]
...
# ── Couche DB unifiée ────────────────────────────────────────
def _pg_connect():
    """Connexion PostgreSQL via pg8000 (pur Python, sans binaires)."""
    import pg8000.native
    from urllib.parse import urlparse
    u = urlparse(DATABASE_URL)
    # Déterminer si SSL requis
    ssl = True  # Vercel Postgres requiert toujours SSL
...
class DB:
    """Abstraction SQLite / PostgreSQL avec curseur dict-like."""
    def __enter__(self):
        if USE_POSTGRES:
            ...
            self._is_pg = False
        self._cur = _PGCursor(self._conn) if self._is_pg else self._conn.cursor()
        return self._cur, self._do_commit
...
class _PGCursor:
    """Curseur pg8000 qui retourne des dicts (clé=colonne, valeur=donnée)."""
    ...
    def execute(self, sql, params=None):
        ...
        try:
            ...
            self._rows = [dict(zip(cols, row)) for row in (result or [])]
        except Exception:
            self._rows = []
    ...
    @property
    def lastrowid(self):
        # pg8000 retourne l'id via RETURNING
        return self._rows[0].get("id") if self._rows else None

def Q(sql):
    """Les ? sont convertis dans _PGCursor.execute pour pg8000."""
    return sql
...
def ensure_db():
    ...
    if request.path in ('/sw.js', '/manifest.json', '/icon-192.png', '/icon-512.png'):
        return
...
<div class="header">
  ...
  <div class="header-actions">
    <div class="user-badge">
      Connecté·e : <strong id="user-display">—</strong>
    </div>
...
function initApp() {
  ...
  document.getElementById('user-display').textContent = currentUser.prenom + ' ' + currentUser.nom;
...
def manifest():
    ...
    data = {
        ...
        "description": "Collecte et analyse de données médicales",
        "start_url": "/",
        "display": "standalone",
...
def register():
    ...
    try:
        with DB() as (cur, commit):
            if USE_POSTGRES:
                cur.execute(
                    "INSERT INTO users (email, password, nom, prenom, created_at) VALUES (?,?,?,?,?) RETURNING id",
                ...
                cur.execute(
                    "INSERT INTO users (email, password, nom, prenom, created_at) VALUES (?,?,?,?,?)",
                    (data["email"].strip(), hash_password(data["password"]),
                     data["nom"].strip(), data["prenom"].strip(),
                     datetime.now().strftime("%d/%m/%Y %H:%M"))
                )

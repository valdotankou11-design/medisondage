...
#  Langage : Python (Flask + SQLite + statistiques)
# =============================================================
...
def build_html():
    """Génère le HTML complet de l'application en Python."""
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .header{background:var(--card);border-bottom:1px solid var(--border);
            ...
            justify-content:space-between;height:64px;position:sticky;top:0;z-index:99}
    ...
    .logo-name{font-size:20px;font-weight:600;letter-spacing:-0.5px}
    ...
    .card-title{font-size:11px;font-weight:700;text-transform:uppercase;
                letter-spacing:.08em;color:var(--muted);margin-bottom:1.25rem}
    ...
    .form-group label{display:block;font-size:13px;color:var(--muted);
                      margin-bottom:6px;font-weight:500}
    .req{color:var(--red)}
    input,select,textarea{width:100%;padding:10px 14px;font-size:14px;
      font-family:inherit;border:1.5px solid var(--border);border-radius:10px;
      background:#fafafa;color:var(--text);outline:none;transition:.15s}
    input:focus,select:focus,textarea:focus{border-color:var(--green);
      background:#fff;box-shadow:0 0 0 3px rgba(29,158,117,.12)}
    textarea{resize:vertical;min-height:90px}
    .radio-group{display:flex;flex-wrap:wrap;gap:10px}
    .radio-label{display:flex;align-items:center;gap:7px;font-size:13px;
      cursor:pointer;padding:7px 14px;border:1.5px solid var(--border);
      border-radius:8px;transition:.15s}
    .radio-label.sel{border-color:var(--green);background:var(--green-l);
                     color:var(--green-d);font-weight:600}
    .scale-row{display:flex;gap:6px;flex-wrap:wrap}
    .scale-btn{width:38px;height:38px;border:1.5px solid var(--border);
      border-radius:8px;background:#fafafa;font-size:13px;cursor:pointer;
    ...
    .btn{padding:12px 24px;border-radius:10px;border:none;font-size:15px;
    ...
    .metric-label{font-size:11px;color:var(--muted);text-transform:uppercase;
                  letter-spacing:.07em;margin-bottom:6px;font-weight:700}
    ...
    .bar-lbl{width:110px;font-size:12px;color:var(--muted);text-align:right;
    ...
    .btn-del{font-size:12px;color:var(--red);border:none;background:none;
    ...
    <div id="formulaire" class="page active">
      ...
      <div id="alert-ok" class="alert alert-ok">✓ Réponse enregistrée avec succès !</div>
      <div id="alert-err" class="alert alert-err">✗ Erreur : veuillez remplir tous les champs obligatoires.</div>
        ...
        <div class="card">
          ...
          <div class="grid2">
            ...
            <div class="form-group">
              ...
              <input id="nom" type="text" placeholder="Ex : Mballa">
            </div>
          </div>
          <div class="grid2">
            <div class="form-group">
              <label>Âge <span class="req">*</span></label>
              <input id="age" type="number" min="1" max="120" placeholder="Ex : 34">
            </div>
            <div class="form-group">
      ...
      <div id="s2" style="display:none">
        <div class="card">
          ...
          <div class="form-group">
            <label>Avez-vous une maladie chronique diagnostiquée ?</label>
            <div class="radio-group" id="rg-maladie">
              <label class="radio-label" onclick="pick(this,'rg-maladie')">
                <input type="radio" name="maladie" value="Oui" style="display:none"> Oui
              </label>
              <label class="radio-label" onclick="pick(this,'rg-maladie')">
                <input type="radio" name="maladie" value="Non" style="display:none"> Non
              </label>
              <label class="radio-label" onclick="pick(this,'rg-maladie')">
                <input type="radio" name="maladie" value="Ne sais pas" style="display:none"> Je ne sais pas
              </label>
            </div>
          </div>
          <div class="form-group">
            <label>Type de pathologie (si applicable)</label>
          ...
          <div class="form-group">
            <label>Fréquence des visites médicales par an</label>
            <select id="visites">
              <option value="">Sélectionner</option>
          ...
          <div class="form-group">
            <label>Évaluez votre état de santé actuel (1 = Très mauvais · 10 = Excellent)</label>
            <div class="scale-row" id="sc-sante"></div>
            <div class="scale-hint"><span>Très mauvais</span><span>Excellent</span></div>
          </div>
        </div>
        <div class="btn-row">
      ...
      <div id="s3" style="display:none">
        <div class="card">
          ...
          <div class="form-group">
            <label>Activité physique hebdomadaire</label>
            <select id="activite">
              <option value="">Sélectionner</option>
          ...
          <div class="form-group">
            <label>Accès à une structure de santé</label>
            <div class="radio-group" id="rg-acces">
              <label class="radio-label" onclick="pick(this,'rg-acces')">
                <input type="radio" name="acces" value="Facile" style="display:none"> Facile
              </label>
              <label class="radio-label" onclick="pick(this,'rg-acces')">
                <input type="radio" name="acces" value="Difficile" style="display:none"> Difficile
              </label>
              <label class="radio-label" onclick="pick(this,'rg-acces')">
                <input type="radio" name="acces" value="Aucun" style="display:none"> Aucun accès
              </label>
            </div>
          </div>
          <div class="form-group">
            <label>Satisfaction envers le système de santé (1 = Très insatisfait · 10 = Très satisfait)</label>
            <div class="scale-row" id="sc-satis"></div>
            <div class="scale-hint"><span>Très insatisfait</span><span>Très satisfait</span></div>
          </div>
          <div class="form-group">
            <label>Observations / Commentaires libres</label>
            <textarea id="commentaire" placeholder="Partagez vos remarques ou suggestions..."></textarea>
          </div>
        </div>
        <div class="btn-row">
    ...
    <div id="dashboard" class="page">
      ...
      <div class="charts-grid">
        ...
        <div class="card">
          ...
          <div class="bar-chart" id="bars-acces"></div>
        </div>
      </div>
      <div class="card">
        <div class="card-title">Pathologies déclarées</div>
        <div class="bar-chart" id="bars-patho"></div>
      </div>
      <div class="card">
        <div class="card-title">Distribution — Score de santé</div>
        <canvas id="hist" width="780" height="160" style="width:100%"></canvas>
      </div>
      ...
      <div id="dash-empty" class="empty" style="display:none">
        Aucune donnée. Soumettez d'abord quelques réponses via le formulaire.
      </div>
    </div>
    ...
    <div id="reponses" class="page">
      ...
      <div class="rep-list" id="rep-list">
        <div class="empty">Aucune réponse pour le moment.</div>
      </div>
    </div>
    """
    ...
    function buildScale(id,cb){
      ...
      for(var i=1;i<=10;i++){
        (function(v){
          ...
          b.className='scale-btn'; b.textContent=v;
          b.onclick=function(){
            c.querySelectorAll('.scale-btn').forEach(x=>x.classList.remove('sel'));
            this.classList.add('sel'); cb(v);
          };
          c.appendChild(b);
        })(i);
      }
    ...
    function step(n){
      if(n===2){
        if(!v('prenom')||!v('nom')||!v('age')||!v('sexe')){
    ...
    function flash(id,msg){
      ...
      if(msg) el.textContent=(id==='alert-ok'?'✓ ':' ✗ ')+msg;
    ...
    function submitForm(){
      var data={
        ...
        region:v('region'), maladie:radio('maladie'),
        pathologie:v('pathologie'), visites:v('visites'),
        sante:scSante, activite:v('activite'),
      ...
      fetch('/api/soumettre',{
        ...
        if(d.success){
          flash('alert-ok','Réponse enregistrée avec succès !');
          resetForm();
        } else {
          flash('alert-err', d.message||'Erreur serveur.');
        }
    ...
    function loadDash(){
      fetch('/api/statistiques').then(r=>r.json()).then(d=>{
        if(!d.total){
          document.getElementById('dash-empty').style.display='block'; return;
        }
        document.getElementById('dash-empty').style.display='none';

        // Métriques
        var mDiv=document.getElementById('metrics');
        mDiv.innerHTML=metric('Total réponses', d.total, 'mv-green', '')+
    ...
    function drawBars(id, counts, total, color){
      ...
      if(!sorted.length) c.innerHTML='<div style="font-size:13px;color:var(--muted)">Aucune donnée</div>';
    }
    ...
    function drawDonut(id, counts, legId){
      ...
      vals.forEach((v,i)=>{
        ...
        ctx.arc(60,60,54,start,start+slice);
        ctx.closePath(); ctx.fillStyle=COLORS[i%COLORS.length]; ctx.fill();
        start+=slice;
      });
      ctx.beginPath(); ctx.arc(60,60,28,0,2*Math.PI);
      ctx.fillStyle='#fff'; ctx.fill();
      ctx.fillStyle='#1a1a1a'; ctx.font='bold 14px DM Sans,sans-serif';
    ...
    function drawHist(id, dist){
      var canvas=document.getElementById(id);
      var ctx=canvas.getContext('2d');
      var W=canvas.offsetWidth||780, H=160;
      canvas.width=W; canvas.height=H;
      ctx.clearRect(0,0,W,H);
      var vals=Object.values(dist).map(Number);
      var maxV=Math.max(...vals,1);
      var bw=Math.floor(W/12), gap=4;
      Object.entries(dist).forEach(([k,v],i)=>{
        var bh=Math.round(v/maxV*(H-40));
        var x=i*(bw+gap)+20, y=H-bh-30;
    ...
    function loadRep(){
      fetch('/api/reponses').then(r=>r.json()).then(rows=>{
        ...
        if(!rows.length){c.innerHTML='<div class="empty">Aucune réponse pour le moment.</div>';return;}
        c.innerHTML=rows.map(r=>'<div class="rep-card">'+
          '<div class="rep-head">'+
            '<span class="rep-name">'+r.prenom+' '+r.nom+'</span>'+
            '<span class="rep-date">'+r.date+'</span>'+
          '</div>'+
          '<div class="rep-detail">'+r.age+' ans &middot; '+r.sexe+' &middot; '+
            (r.region||'—')+' &middot; Santé : '+r.sante+'/10 &middot; Pathologie : '+r.pathologie+'</div>'+
          '<div class="rep-actions">'+
            '<button class="btn-del" onclick="delRep('+r.id+',this)">Supprimer</button>'+
          '</div></div>'
        ).join('');
      });
    }
    ...
    function delRep(id,btn){
      ...
      fetch('/api/supprimer/'+id,{method:'DELETE'}).then(()=>{
    ...
    // ── INIT ──
    buildScale('sc-sante',v=>scSante=v);
    buildScale('sc-satis',v=>scSatis=v);
    """
...
def statistiques():
    conn = get_db()
    rows = [dict(r) for r in conn.execute("SELECT * FROM reponses").fetchall()]
    conn.close()
    if not rows:
...
def liste_reponses():
    conn = get_db()
    rows = conn.execute("SELECT * FROM reponses ORDER BY id DESC LIMIT 100").fetchall()
    conn.close()
...
def export_csv():
    ...
    writer.writerow([
        "ID","Date","Prénom","Nom","Âge","Sexe","Région",
...
if __name__ == "__main__":
    init_db()
    print("\n" + "="*52)
    print("   MédiSondage — INF 232 EC2 — Analyse de données")
